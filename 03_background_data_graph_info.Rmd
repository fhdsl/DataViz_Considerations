...
An example of a conventional plot type that uses **lines** is a line plot. Lines connect different data points. Slope may be a value of interest when working with lines, but it's often difficult for audiences to compare slope values visually without labels. Lines may also be trendlines that aren't connecting explicit data points and rather are added to a plot to show a general relationship or trend. Lines are often used to add annotations for summary statistics such as averages as well as medians or quartiles (in boxplots).
...
**When to use**:
  ...
  * Examples uses include tracking patient response or symptom trajectories as well as patient cohort stratification
